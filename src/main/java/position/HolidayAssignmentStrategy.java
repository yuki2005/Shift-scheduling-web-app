...
@Component
public class HolidayAssignmentStrategy
    ...
    public Map<Pos, List<Employee>> assign(
        ...
        // Dに総合能力最大を割り当てる
        if (requiredCount.getOrDefault(Pos.D, 0) > 0) {
            ...
            if (best != null) {
                result.computeIfAbsent(Pos.D, k -> new ArrayList<>()).add(best);
                assigned.add(best);
                requiredCount.put(Pos.D, requiredCount.get(Pos.D) - 1);
            }
        }
